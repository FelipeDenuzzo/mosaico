...
import importlib
try:
    importlib.import_module("pillow_avif")  # registra suporte AVIF no Pillow
except Exception:
    pass
...
def _resolver_caminho_tile(path: str) -> str:
    """Tenta resolver o caminho físico do tile.
    Se o caminho absoluto gravado no SQLite não existir localmente,
    tenta encontrá-lo dentro da pasta 'acervo' do projeto.
    """
    if os.path.exists(path):
        return path
    nome_arquivo = os.path.basename(path)
    caminho_local = os.path.join(PROJECT_ROOT, "acervo", nome_arquivo)
...
# --------------------------------------------------
RECENCIA_BONUS_MAX = 1000       # desconto máximo na distância² de cor (aumentado para priorizar novas fotos)
RECENCIA_TOP_N = 100            # nº de candidatos onde o bônus atua (aumentado para permitir fotos novas)
...
class TerminalProgress:
    ...
    def update(self, valor: int) -> None:
        ...
        # Throttling: só escreve se o percentual inteiro mudar ou se for o último
        perc_antigo = int((self.current / self.total) * 100)
        perc_novo = int((novo / self.total) * 100)
        
        self.current = novo
        
        if perc_novo == perc_antigo and novo != self.total:
            return
...
def _preaquecer_cache_tiles_renderizados(
    ...
    for path, _rgb, _created_at in descritores_catalogo:
        try:
            caminho_resolvido = _resolver_caminho_tile(path)
            with Image.open(caminho_resolvido) as img:
...
def selecionar_pixel(
    ...
    def _escolher_com_variacao(lista_candidatos):
        pool_size = max(1, variacao_cor)
        # Pega os melhores por cor
        top_color = nsmallest(
            pool_size,
            lista_candidatos,
            key=lambda p: _distancia_cor_quadrada(cor_alvo, p.average_color),
        )
        # Se a variacao_cor for grande, embaralhamos levemente os melhores
        # para quebrar o determinismo, mas ainda respeitando a recência
        top_color.sort(key=_score_recencia)
        # Sorteia entre os top 20% da pool para dar variedade
        limite = max(1, int(len(top_color) * 0.2))
        return random.choice(top_color[:limite])
    ...
    if candidatos_ambas:
        return _escolher_com_variacao(candidatos_ambas)
    ...
    if candidatos_max_uses:
        ...
        return _escolher_com_variacao(candidatos_max_uses)
    ...
    return _escolher_com_variacao(menos_usados)
...
def _renderizar_faixa(
    ...
    recent_tracker: Optional[Dict[str, Any]] = None,
...
def criar_mosaico(
    caminho_base: str,
    tamanho_pixel: int = FIXED_TILE_SIZE,
    redimensionar: bool = True,
    max_repeticoes: int = FIXED_MAX_REPETITIONS,
    variacao_cor: int = FIXED_COLOR_VARIATION,
    caminho_saida: str = "",
    qualidade: int = 85,
    ...
    """
    if isinstance(tamanho_pixel, str) and not caminho_saida:
        caminho_saida = tamanho_pixel
        tamanho_pixel = FIXED_TILE_SIZE
    ...
    # Forçar o uso das variáveis fixas do topo do arquivo
    variacao_cor_efetiva = max(0, min(100, int(FIXED_COLOR_VARIATION)))
    max_repeticoes_efetivo = max(0, int(FIXED_MAX_REPETITIONS))
    ...
    try:
        if usar_bandas:
            ...
            try:
                ...
                progresso_montagem.close()
                terminal_log("Montagem concluida.", arquivo=nome_arquivo_base)
                terminal_log("Inicio da gravacao final do mosaico (JPEG).", arquivo=nome_arquivo_base)
                t_export = time.perf_counter()
                mosaico.save(caminho_saida, "JPEG", quality=90)
                metricas_tempo["exportacao_jpeg_s"] = time.perf_counter() - t_export
                production_log(f"TEMPO_SALVAR_JPEG_S={metricas_tempo['exportacao_jpeg_s']:.3f}")
                terminal_log("Exportacao concluida (JPEG).", arquivo=nome_arquivo_base)
            finally:
                mosaico.close()
    except Exception as exc:
        progresso_montagem.close()
        terminal_log(f"Erro na montagem do mosaico: {exc}", level="ERROR", arquivo=nome_arquivo_base)
        raise
    ...
    if recent_tracker["col"] != -1 and recent_tracker["row"] != -1:
        try:
            ...
            debug_log(f"Erro ao salvar JSON do recent_tracker: {exc_json}")
