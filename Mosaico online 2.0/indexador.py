...
PASTA_TEMPORARIA_TILES = os.path.join(BASE_DIR, "archive")
...
# ==============================================================================
# FUNIL: ARCHIVE -> ACERVO (400x400) -> DB
# ==============================================================================
...
def processar_pasta_temporaria(conn: sqlite3.Connection) -> int:
    """
    Processa imagens de archive, gera tiles 400x400 em acervo,
    indexa no tiles_index.db e apaga o original de archive após sucesso.
    """
    ...
    for caminho_tmp in arquivos:
        ...
        try:
            ...
            with Image.open(caminho_tmp) as img:
                ...
                # 3. Redimensionar para 400x400 (padrão do acervo)
                img_final = img_cropped.resize((400, 400), Image.Resampling.LANCZOS)
                if LOG_DETALHADO_INDEXACAO:
                    print("  - resize=400x400")
...
def remover_tiles_orfaos(conn: sqlite3.Connection) -> int:
    ...
    Se o arquivo foi movido ou a pasta raiz mudou, tenta resolver o caminho localmente
    e atualiza o registro no banco em vez de deletar.
    """
    cur = conn.execute("SELECT id, path, filename FROM tiles")
    rows = cur.fetchall()

    removidos = 0
    atualizados = 0
    base_dir = os.path.dirname(os.path.abspath(__file__))
    acervo_local = os.path.join(base_dir, "acervo")

    for tile_id, path, filename in rows:
        if os.path.exists(path):
            continue
            
        # Tenta resolver o caminho localmente na pasta 'acervo' do projeto
        caminho_local = os.path.join(acervo_local, filename)
        if os.path.exists(caminho_local):
            # O arquivo existe localmente! Atualiza o banco com o caminho correto da máquina atual
            conn.execute("UPDATE tiles SET path = ? WHERE id = ?", (caminho_local, tile_id))
            atualizados += 1
        else:
            # O arquivo realmente sumiu de todos os lugares
            conn.execute("DELETE FROM tiles WHERE id = ?", (tile_id,))
            removidos += 1

    conn.commit()
    if atualizados > 0:
        print(f"[INDEXADOR] Atualizados {atualizados} caminhos de tiles para a pasta local.")
    if removidos > 0:
